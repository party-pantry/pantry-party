...
export async function POST(req: Request) {
  try {
    ...
    // Begin moved addStock logic (minimal changes)
    ...
    if (!storage) {
      return NextResponse.json({ error: `Storage with ID ${storageId} does not exist` }, { status: 404 });
    }
    ...
    if (ingredient.foodCategory === 'MEAT' && (storage.type !== 'FREEZER' && storage.type !== 'FRIDGE')) {
      return NextResponse.json({ error: 'Meat ingredients must be stored in a Freezer or a Fridge' }, { status: 401 });
    }
    if (ingredient.foodCategory === 'DAIRY' && storage.type !== 'SPICE_RACK') {
      return NextResponse.json({ error: 'Dairy ingredients cannot be stored in a Spice Rack' }, { status: 401 });
    }
    if (ingredient.foodCategory === 'FROZEN' && storage.type !== 'FREEZER') {
      return NextResponse.json({ error: 'Frozen ingredients must be stored in a Freezer' }, { status: 401 });
    }
    if (ingredient.foodCategory === 'PRODUCE' && (storage.type !== 'FRIDGE' && storage.type !== 'PANTRY')) {
      return NextResponse.json({ error: 'Produce ingredients must be stored in a Fridge or a Pantry' }, { status: 401 });
    }
