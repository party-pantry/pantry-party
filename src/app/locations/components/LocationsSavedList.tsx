...
const LocationsSavedList: React.FC<Props> = ({ saved = [], onSelectResult, onRemoveSaved }) => {
    ...
    return (
        <ListGroup variant="flush">
            {saved.map((p) => (
                <ListGroup.Item key={p.id || `${p.label}-${p.latitude}-${p.longitude}`} className="py-2 border-bottom d-flex justify-content-between align-items-center">
                    <Button variant="link" className="text-start p-0 flex-grow-1 text-wrap" onClick={() => onSelectResult?.(p)}>
                        ...
                        <div className="text-muted small">{p.address || (p.latitude != null && p.longitude != null ? `${p.latitude.toFixed(5)}, ${p.longitude.toFixed(5)}` : '')}</div>
                    </Button>
                    <div className="ms-2">
