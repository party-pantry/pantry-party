...
const LocationsResults: React.FC<Props> = ({ loading, suggestions, saved = [], onSelectResult, onSave }) => {
  ...
  return (
      ...
      {suggestions.map((r) => {
        ...
        const itemKey = r.id || `${r.label}-${r.latitude}-${r.longitude}`;
        const addressDisplay = r.address || (r.latitude != null && r.longitude != null
          ? `${r.latitude.toFixed(5)}, ${r.longitude.toFixed(5)}`
          : 'Unknown location');
        return (
          <ListGroup.Item key={itemKey} className="py-2 border-bottom">
